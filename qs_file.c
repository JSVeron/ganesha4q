...
///////////////
FHResult QsFileSystem::lookupFileHandle(QsFileHandle* parent, const char *name, uint32_t flags) {
...
FHResult QsFileSystem::touchFile(QsFileHandle* parent, const char *name,
...
FHResult QsFileSystem::mkDir(QsFileHandle* parent, const char *name,
            struct stat *st, uint32_t mask)
{
    if(!parent->isDir())
    {
...
FHResult QsFileSystem::createFileObj(QsFileHandle* parent, const char *name,
            struct stat *st, uint32_t mask)
{
    // qingstor_file_handle *lfh;
    // with QS_FS_FLAG_NONE means do not notify backend storage(QingStor) to create obj.
    FHResult result = lookupFileHandle(parent, name, QSFileHandle::QS_FS_FLAG_NONE);
    ...
    if(QsFsError::QS_FS_ERR_NO_ERROR == err){
        ...
        return FHResult(QS_FS_ERR_FILE_ALEADY_EXIST);
    }
    else if (QsFsError::QS_FS_ERR_NTWORK_ERROR == err) {
        // log here
        return FHResult(QS_FS_ERR_NTWORK_ERROR);
    }
    ...
    if(QsFsError::QS_FS_ERR_FILE_NO_EXIST == err){
        // we can create the dir now
          ...
          {
              // log here
              fhResult.err = QsFsError::QS_FS_ERR_NTWORK_ERROR;       
          }
          ...
          if( HttpResponseCode::OK == respCode){
              ...
              // log here
              return FHResult(QsFsError::QS_FS_ERR_UNKNOWN);
          }
    }
           ...
           std::string srcName, std::string dstName)
{
    ...
    /* atomicity FLAG_LOCK*/
    FHResult result = lookupFileHandle(srcParent, srcName, QSFileHandle::QS_FS_FLAG_NONE);
